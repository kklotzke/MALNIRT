...
#nu1 <- c(seq(0.2, -0.05, length.out = 6), seq(-0.1, -0.2, length.out = 8), seq(-0.3, -0.4, length.out = 6))
#nu2 <- c(seq(-0.05, 0.2, length.out = 7), c(0, -0.2, -0.3, -0.25, 0.1, 0.05, -0.1), seq(-0.15, 0.2, length.out = 6))
#nu1 <- rep(-0.1, sim.K)
#nu2 <- rep(-0.2, sim.K)
sim.N <- 250
sim.K <- 30
...
nu1 <- runif(min = -0.2,max = 0.2, n = sim.K)
nu2 <- runif(min = -0.2,max = 0.2, n = sim.K)
delta1 <- c(0.2, 0)
delta2 <- c(0.3, 0)
tau1 <- c(0.2, 0)
tau2 <- c(0.3, 0)
...
save(out, delta1, delta2, tau1, tau2, nu1, nu2, theta2, zeta2, dat1, dat2, file = "simulation2_280317_small.Rdata")

#load("simulation2_280317_small.Rdata")
...
sink("simulation2_280317_small.txt")
