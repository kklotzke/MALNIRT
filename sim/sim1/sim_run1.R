...
sim.mh.accept1 <- matrix(0, nrow = 2, ncol = 3)
...
system.time({
  ...
  {
    print(ii)
    dat1 <- simdataLNIRTP(N = sim.N, K = sim.K, delta = delta1_1[1], tau = tau1_1[1], nu = nu1_1)#, beta = beta, lambda = lambda)
    dat2 <- simdataLNIRTP(N = sim.N, K = sim.K, delta = delta1_2[1], tau = tau1_2[1], nu = nu1_2, beta = dat1$beta, lambda = dat1$lambda,
    ...
    if(!is.null(out)) {
      ...
      sim.beta1[ii, ] <- (out$post.means[[1]]$beta + out$post.means[[2]]$beta)/2
      sim.lambda1[ii, ] <- (out$post.means[[1]]$lambda + out$post.means[[2]]$lambda)/2
      ...
      sim.mh.accept1 <- sim.mh.accept1 + out$mh.accept

      out.list1[[ii]] <- out
      tmp.mh1 <-sim.mh.accept1/ii
      save(out.list1, sim.tau1_1, sim.delta1_1, sim.nu1_1, sim.tau1_2, sim.delta1_2, sim.nu1_2, sim.theta1_2, sim.zeta1_2,
           sim.cor.beta1, sim.cor.lambda1, sim.cor.sig2k1_1, sim.cor.sig2k1_2, tmp.mh1,
           tau1_1, delta1_1, nu1_1, tau1_2, delta1_2, nu1_2, theta1_2, zeta1_2, file = "simulation1_140417_1.RData")
      ii <- ii + 1
    }
  }
})
