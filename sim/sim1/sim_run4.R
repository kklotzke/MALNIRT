...
sim.mh.accept4 <- matrix(0, nrow = 2, ncol = 3)
...
system.time({
  ...
  {
    print(ii)
    dat1 <- simdataLNIRTP(N = sim.N, K = sim.K, delta = delta4_1[1], tau = tau4_1[1], nu = nu4_1)#, beta = beta, lambda = lambda)
    dat2 <- simdataLNIRTP(N = sim.N, K = sim.K, delta = delta4_2[1], tau = tau4_2[1], nu = nu4_2, beta = dat1$beta, lambda = dat1$lambda,
    ...
    if(!is.null(out)) {
      ...
      sim.mh.accept4 <- sim.mh.accept4 + out$mh.accept

      out.list4[[ii]] <- out
      tmp.mh4 <-sim.mh.accept4/ii
      save(out.list4, sim.tau4_1, sim.delta4_1, sim.nu4_1, sim.tau4_2, sim.delta4_2, sim.nu4_2, sim.theta4_2, sim.zeta4_2,
           sim.cor.beta4, sim.cor.lambda4, sim.cor.sig2k4_1, sim.cor.sig2k4_2, tmp.mh4,
           tau4_1, delta4_1, nu4_1, tau4_2, delta4_2, nu4_2, theta4_2, zeta4_2, file = "simulation1_140417_4.RData")
      ii <- ii + 1
    }
  }
})
