...
system.time({
  ...
  while (ii <= sim.rep)
  {
    ...
    if(!is.null(out)) {
      ...
      sim.beta4[ii, ] <- out$post.means[[1]]$beta
      sim.lambda4[ii, ] <- out$post.means[[1]]$lambda
      ...
      save(out.list1, sim.tau4_1, sim.delta4_1, sim.nu4_1, sim.tau4_2, sim.delta4_2, sim.nu4_2, sim.theta4_2, sim.zeta4_2,
           ...
           tau4_1, delta4_1, nu4_1, tau4_2, delta4_2, nu4_2, theta4_2, zeta4_2, file = "simulation4_250317_4.RData")
      ii <- ii + 1
    }
  }
})
