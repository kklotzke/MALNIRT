...
#' @export
MALNIRT_ZT <- function(Y, RT, group = NULL, data, XG = 1000, XG.init = 100, burnin = 0.10, est.person = FALSE,
  ...
  sampleProposals <- function(reinit = FALSE, g, e) {
    ...
    e$param[[g]]$nu.cand <- diag(cov(e$param[[g]]$Z, e$RTg[[g]]))  #rep(0, K) out.cor$nu # # diag(cov(e$param[[g]]$Z, e$RTg[[g]]))
    e$param[[g]]$q.nu <- NULL#out.cor$q.nu
  ...
  doMH <- function(e)
  {
    ...
    if(validZT) {
      lik_ZT <- 0
      for (i in 1:Ng) {
        lik_ZT <- lik_ZT + sum(mvnfast::dmvn(ZT[i, ], mu.ZT[i, ], Sigma.ZT, log = TRUE))
      }
    ...
    if(!reset) {
      ...
      if(validZT) {
        lik_ZT.cand <- 0
        for (i in 1:Ng) {
          lik_ZT.cand <- lik_ZT.cand + sum(mvnfast::dmvn(ZT.cand[i, ], mu.ZT.cand[i, ], Sigma.ZT.cand, log = TRUE))
        }
      ...
      if(!is.null(out.Z.cand)) {
        ...
        lik <- lik_ZT + invgamma::dinvgamma(e$param[[g]]$tau.cand + 1/K, e$param[[g]]$q.tau$shape, e$param[[g]]$q.tau$rate, log = TRUE) + invgamma::dinvgamma(e$param[[g]]$delta.cand + 1/K, e$param[[g]]$q.delta$shape, e$param[[g]]$q.delta$rate, log = TRUE)
        lik.cand <- lik_ZT.cand + invgamma::dinvgamma(e$tau.s0 + 1/K, e$param[[g]]$q.tau$shape, e$param[[g]]$q.tau$rate, log = TRUE) + invgamma::dinvgamma(e$delta.s0 + 1/K, e$param[[g]]$q.delta$shape, e$param[[g]]$q.delta$rate, log = TRUE)
