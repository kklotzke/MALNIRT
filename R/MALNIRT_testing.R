...
#' @export
MALNIRT3 <- function(Y, RT, Group = NULL, data, XG = 1000, burnin = 0.10, inits.1 = NULL, inits.2 = NULL, hyper.priors = NULL, est.person = FALSE, silent = FALSE) {
  ...
  for (cc in 1:2) {
    ...
    ZT <- ZT.cand <- matrix(rnorm(N*K, 0, 1), ncol = K, nrow = N) # Z|T
    ZT2 <- ZT2.cand <- matrix(0, ncol = K, nrow = N) # Zk|Z.mink, T1..p
    ...
    for (ii in 2:XG) {
      ...
      # Conditional mean of Z|T for each item
      mu_ZT.cand <- matrix(0, ncol = K, nrow = N)
      x <- matrix(NA, ncol = K, nrow = N)
      nu.cand <- numeric(K)
      for (k in 1:K)
      {
        ### Sample o_k ###
        nu.cand[k] <- runif(1, nu.s0[k] - 0.10, nu.s0[k] + 0.10)

        x[, k] <- (1/sig2k.s0[k]) * ((RT[, k] - mu_T[k]) - (RT[, k] - mu_T[k]) / (a * sig2k.s0[k]))
        mu_ZT.cand[, k] <- mu_Z[k] + nu.cand[k] * x[, k]
      }
      ...
      lik_ZT2.cand <- numeric(K)
      nuk.accept <- numeric(K)
      for (k in 1:K)
      {
        w.min1 <- nu.s0[-k]/sig2k.s0[-k]
        b.min1 <- sig2k.s0[-k] / (sig2k.s0[-k] - nu.s0[-k]^2)
        ...
        B11 <- 1 + tau.s0[1] - nu.cand[k]^2 * (1/sig2k.s0[k] - ((1/sig2k.s0[k])^2) / a)
        B22 <- (1 - (nu.s0[-k]^2) / sig2k.s0[-k])*I.min1 + tau.s0[1]*J.min1 + w.min1 %*% t(w.min1) / a
        B12 <- tau.s0[1]*ones.min1 + ((nu.cand[k] / sig2k.s0[k]) %*% t(w.min1)) / a
        B21 <- t(B12)
        ...
        # Sample Zk|Z.mink, T1..p
        boundary <- 0
        ZT2.cand[Y[,k]==0, k] <- truncnorm::rtruncnorm(n = N, mean = mu_ZT2[, k], sd = sqrt(var_ZT2[k]), a = -Inf, b = boundary)[Y[, k]==0]
        ZT2.cand[Y[,k]==1, k] <- truncnorm::rtruncnorm(n = N, mean = mu_ZT2[, k], sd = sqrt(var_ZT2[k]), a = boundary, b = Inf)[Y[, k]==1]
        #muz <- mu_ZT2[, k]
        #sz <- sqrt(var_ZT2[k])
        #ZT2[Y[,k]==0,k]<-qnorm(runif(N,0,pnorm(0,muz,sz)),muz,sz)[Y[,k]==0]
        #ZT2[Y[,k]==1,k]<-qnorm(runif(N,pnorm(0,muz,sz),1),muz,sz)[Y[,k]==1]

        r[, k] <- (ZT2.cand[, k] - mu_Z[k]) - tmp[1, ]
        s[, k] <- ZT2.cand[, k] - tmp[1, ] - nu.cand[k]*x[,k]
      ...
      ### Sample covariance responses-response times ###
      for (k in 1:K) {
        #SS = sum((r[, k] - nu.cand[k]*x[i, k])^2)
        SS = sum((ZT2.cand[k] - mu_ZT2[k])^2)
        #lik_ZT2.cand[k] <- (2 * pi * var_ZT2[k])^(-N/2) * exp(-SS/(2*var_ZT2[k]))
        lik_ZT2.cand[k] <- (-N/2) * (2 * pi * var_ZT2[k]) -SS/(2*var_ZT2[k])
        u <- runif (1, 0, 1)
        #ar <- lik_ZT2.cand[k] / lik_ZT2
        ar <- exp(lik_ZT2.cand[k] - lik_ZT2[k])
        if (is.nan(ar))
          ar <- 0
#browser()
        if (u <= ar) { # Accept
          ...
          chain[[17]][ii, k] <- nu.s0[k]
          nu.accept <- c(nu.accept, 0)
        }
      }
      #print(lik_ZT2.cand)
